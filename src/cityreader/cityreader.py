...
def cityreader(cities=[]):
    ...
    with open('cities.csv', 'r') as cities_csv:
        cities_content = csv.reader(cities_csv)
        next(cities_content)
        for city in cities_content:
            name, lat, lon = (city[0], float(city[3]), float(city[4]))
...
try:
    lat1, lon1 = input("Enter latitude1,longitude1: ").split(",")
    lat2, lon2 = input("Enter latitude2,longitude2: ").split(",")
except ValueError:
    print("Expected 2 Values latitude, longitude")
